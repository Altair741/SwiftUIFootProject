...
struct QListView: View {
    ...
    let taskList: [QuestionItem] = [
        ...
        QuestionItem(task: "Risk Calculator", view: AnyView(RiskCalculator()))
    ]
                                        ...
                                        ForEach(taskList) { question in
                                            Button(action: {
                                                ...
                                                HStack { // Wrap with HStack for better layout
                                                    ...
                                                    if let index = taskList.firstIndex(where: { $0.id == question.id }),
                                                       index < answer.answerRecord.count, // Ensure the index is within bounds
...
//                                                        }
                ...
                HStack {
                    Button(action: {
                        ...
                        if currentViewOrder == 0 {
                            // if first page, not showing back button
                        }
                        else{
                            Text("Back")
                                .padding()
                    ...
                    Button(action: {
                        withAnimation {
                            ...
                            else if taskList[currentViewOrder].task == "Risk Calculator" {
                                print("in risk calculate ..." )
                            }
                            
                            else if taskList[currentViewOrder].task == "MyModal"
                            {
                                if(checkMedicalToolAnswer()){
    ...
    func checkMedicalToolAnswer() -> Bool
    {
        // check if the user make selection
        if answer.haveMedicalTool == "Not Answered"
        {
            showAlert = true
            print("checking medical tool answer...")
            return false
        }
        return true
    }
