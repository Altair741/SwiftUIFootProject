...
struct EndPage: View {
    ...
    @State private var goTestAgain = false
    ...
    var body: some View {
        ...
        VStack
        {
            ...
            HStack{
                Button(action: {
                    answer.reset()// Reset user answer
                    DispatchQueue.main.asyncAfter(deadline: .now() + 0.4) {
                            goTestAgain = true
                        }
                    
                    
                }, label: {Text("Test again")}).fullScreenCover(isPresented: $goTestAgain) {
                    NavigationStack { // 새로운 NavigationStack 생성
                        LaunchScreen()
                    }
                }.padding()
                ...
                .sheet(isPresented: $sendMail) {
                    if MFMailComposeViewController.canSendMail() {
                        EmailSender(
                            ...
                            messageBody:"""
                            The Assessment has done on the date \(currentDate) and it took \(Int(answer.assessmentTime)) seconds, and patient risk level generated by system is \(answer.system_g_risk)
